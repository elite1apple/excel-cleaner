...
headers = ['Tag/Unit', 'Drawing Ref.', 'Q', 'Product', 'Roll', 'Width', 'Height', 'Chain Length', 'Fabric', 'Mount', 'Control', 'Deductions', 'Unnamed: 12']
...
sample_data = [
    ...
    ['', '', 1, 'Manual Shade', 'Rev', 42.25, 122, 72, 'Liv', '', 'L', 'Dl', 0.5],
    ['', '', 1, 'Manual Shade', 'Rev', 52, 122, 72, 'Liv', '', 'R', 'Dr', 0.5],
    [121, '', 1, 'Manual Shade', '', 48, 96, 60, 'Bed', '', 'L', '', ''],
    ['', '', 1, 'Manual Shade', '', 55.5, 96, 60, 'Bed', '', 'R', '', ''],
    [122, '', 1, 'Manual Shade', 'Rev', 36, 108, 72, 'Liv', '', 'L', 'D', 1.0],
    [223, '', 1, 'Manual Shade', '', 44, 84, 48, 'Studio', '', 'L', '', ''],
    [224, '', 1, 'Manual Shade', '', 50, 90, 60, 'Kitchen', '', 'R', 'Dl', 0.5],
]
...
for row_idx, row_data in enumerate(sample_data, start=10):
    for col_idx, value in enumerate(row_data, start=1):
        ...
        cell.value = value
        cell.border = border
        if col_idx in [6, 7, 8, 13]:  # Numeric columns
            cell.alignment = Alignment(horizontal='right')
        else:
            cell.alignment = Alignment(horizontal='center')

# Set column widths (adjusted for notes in column A)
column_widths = [50, 12, 5, 15, 6, 10, 10, 10, 10, 8, 10, 10, 12]  # Column A wider for notes
...
instructions = [
    ...
    '   • Unnamed: 12: Numeric deduction value (e.g., 0.5, 1.0)',
    '',
    '5. IMPORTANT NOTES:',
...
# Save file
wb.save('STANDARD_TEMPLATE_NEW.xlsx')
